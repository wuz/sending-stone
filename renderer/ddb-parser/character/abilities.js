...
function getCustomSaveProficiency(data, ability) {
  ...
  if (data.character.characterValues) {
    const customProficiency = data.character.characterValues.find(
      (value) =>
        value.typeId === 41 && value.valueId == ability.id && value.value
    );
...
function getCustomSaveBonus(data, ability) {
  ...
  if (data.character.characterValues) {
    const customBonus = data.character.characterValues
      .filter(
        (value) =>
          (value.typeId == 40 || value.typeId == 39) &&
          value.valueId == ability.id
      )
      .reduce((total, bonus) => {
...
function parseAbilities(data, includeExcludedEffects = false) {
  ...
  DICTIONARY.character.abilities.forEach((ability) => {
    ...
    const cappedBonus = utils
      ...
      .reduce(
        (prev, cur) => {
          const restricted = cur.restriction
            ? cappedBonusExp.exec(cur.restriction)
            : undefined;
    ...
    const maxAdjustedStat =
      Math.min(cappedBonus.cap, calculatedStat) + bonusStat;
    // some items will set the ability score if lower
    const setAbilityState =
      maxAdjustedStat > setAbility ? maxAdjustedStat : setAbility;
    ...
    const proficient = customProficiency
      ? customProficiency
      : utils.filterBaseModifiers(
          data,
          "proficiency",
          `${ability.long}-saving-throws`,
          [null, ""],
          includeExcludedEffects
        ).length > 0
      ? 1
  ...
  DICTIONARY.character.abilities.forEach((ability) => {
    const checkBonusModifiers = utils.filterBaseModifiers(
      data,
      "bonus",
      `${ability.long}-ability-checks`,
      [null, ""],
      includeExcludedEffects
    );
    ...
    const saveBonusModifiers = utils.filterBaseModifiers(
      data,
      "bonus",
      `${ability.long}-saving-throws`,
      [null, ""],
    ...
    const modifiersSaveBonus = utils.getModifierSum(
      saveBonusModifiers,
      character
    );
    ...
    if (
      modifiersSaveBonus &&
      modifiersSaveBonus !== "" &&
      parseInt(modifiersSaveBonus)
    ) {
      if (customSaveBonus) {
        const totalSave =
          parseInt(customSaveBonus) + parseInt(modifiersSaveBonus);
      ...
      if (customSaveBonus) {
        result[
          ability.value
        ].bonuses.save = `${modifiersSaveBonus} + ${customSaveBonus}`;
      } else {
        result[ability.value].bonuses.save = `${modifiersSaveBonus}`;
      }
