    ...
    .filter(
      (action) =>
        ...
        (action.limitedUse.maxUses ||
          action.limitedUse.statModifierUsesId ||
          action.limitedUse.useProficiencyBonus)
    );
...
function getSortedByUsedResourceList(data, character) {
  ...
  const resources = allResources
    .map((action) => {
      let maxUses =
        action.limitedUse.maxUses && action.limitedUse.maxUses !== -1
          ? action.limitedUse.maxUses
          : 0;
      ...
      if (action.limitedUse.statModifierUsesId) {
        ...
        switch (action.limitedUse.operator) {
          case 2: {
            maxUses *=
              character.flags.ddbimporter.dndbeyond.effectAbilities[ability]
                .mod;
            break;
          }
          case 1:
...
export function getResources(data, character, numberOfResources = 3) {
  ...
  const resourceSelectionType = hasProperty(
    character,
    "flags.ddbimporter.resources"
  )
  ...
  switch (resourceSelectionType.type) {
    case "custom": {
      const customResourceSelection = getProperty(
        character,
        "flags.ddbimporter.resources"
      );
      for (let i = 0; i < sheetResources.length && i < numberOfResources; i++) {
        ...
        const resource =
          resourceLookupName && resourceLookupName !== ""
...
function generateResourceSelectionFromForm(formData, type) {
  const primary = formData.find(
    (r) => r.name === "primary-select" && r.value !== ""
  );
  const secondary = formData.find(
    (r) => r.name === "secondary-select" && r.value !== ""
  );
  const tertiary = formData.find(
    (r) => r.name === "tertiary-select" && r.value !== ""
  );
...
export async function getResourcesDialog(currentActorId, ddb, character) {
  ...
  return new Promise((resolve) => {
    let currentResourceSelection = hasProperty(
      currentActor,
      "data.flags.ddbimporter.resources.type"
    )
    ...
    if (currentResourceSelection.type === "default") {
      currentResourceSelection = setDefaultResources(
        sortedResources,
    ...
    if (
      currentResourceSelection.ask ||
      !hasProperty(currentResourceSelection, "ask")
    ) {
      ...
      const dialog = new Dialog(
        {
          ...
          buttons: {
            default: {
              ...
              callback: async () => {
                const formData = $(
                  ".character-resource-selection"
                ).serializeArray();
                let resourceSelection = generateResourceSelectionFromForm(
                  formData,
                  "default"
                );
                resourceSelection = setDefaultResources(
                  resources,
                  resourceSelection
                );
            ...
            custom: {
              ...
              callback: async () => {
                ...
                const resourceSelection = generateResourceSelectionFromForm(
                  formData,
                  "custom"
                );
                character = setResourceType(ddb, character, resourceSelection);
