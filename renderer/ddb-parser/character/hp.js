...
export function getHitpoints(data, character) {
  const constitutionHP =
    character.effectAbilities.con.mod * character.totalLevels;
  ...
  const bonusHitPointFeatures = utils.filterBaseModifiers(
    data,
    "bonus",
    "hit-points-per-level"
  );
  const bonusHitPointFeaturesWithEffects = utils.filterBaseModifiers(
    data,
    "bonus",
    "hit-points-per-level",
    ["", null],
  ...
  const bonusHitPointValuesWithEffects = bonusHitPointFeaturesWithEffects.map(
    (bonus) => {
      const cls = utils.findClassByFeatureId(data, bonus.componentId);
      if (cls) {
        return cls.level * bonus.value;
  ...
  const totalBonusHitPoints = bonusHitPointValues.reduce(
    (prev, cur) => prev + cur,
    0
  );
  const totalBonusHPWithEffects = bonusHitPointValuesWithEffects.reduce(
    (prev, cur) => prev + cur,
    0
  );
