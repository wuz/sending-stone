...
export function getClassFeatures(cls, classLevel = 20) {
  if (
    ...
    const subclassFeatures = cls.subclassDefinition.classFeatures.map(
      (subclassFeature) => {
...
function isStartingClass(data, className) {
  return data.classes.find(
    (cls) => cls.definition.name === className && cls.isStartingClass
  );
}
...
function getClassModifiers(data, classFeatures, isStartingClass = false) {
  const modifiers = data.modifiers.class.filter((classModifier) => {
    ...
    if (feature !== undefined) {
      const isFeatureAvailable = classModifier.availableToMulticlass
        ? true
        : isStartingClass;
      console.log(
        `${
          isFeatureAvailable ? "  [  AVAIL]" : "  [UNAVAIL]"
        } Modifier found: ${classModifier.friendlyTypeName} (${
...
function getClassOptionModifiers(data) {
  ...
  const modifiers = data.modifiers.class.filter((classModifier) => {
    ...
    if (feature === undefined) {
      console.log(
        `Modifier found: ${classModifier.friendlyTypeName} (${classModifier.friendlySubtypeName})`
      );
      return true;
    }
    return false;
...
function filterModifiers(data, classInfo) {
  ...
  data.classes.forEach((cls, index) => {
    ...
    classInfo[index].modifiers = getClassModifiers(
      data,
      features,
      isStartingClass(data, cls.definition.name)
    );
  });
  return classInfo;
}
...
export function fixCharacterLevels(data) {
  ...
  filteredClassInfo.forEach((cls) => {
    console.log(
      `${cls.isStartingClass ? "Starting Class" : "Multiclass"}: [lvl${
        cls.level
      }] ${cls.name} `
    );
    console.log(
